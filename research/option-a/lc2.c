...
int main(int argc, char**argv){
  ...
  for(uint32_t i=0;i<ic;i++){
    ...
    const uint8_t*lc=mh+32; int n=0;
    printf("%s ncmds=%u\n",path,ncmds);
    for(uint32_t j=0,off=0;j<ncmds&&off+8<=sz;j++){
      ...
      uint32_t base=cmd & 0x7fffffff;
      if(base==0xc||base==0x18||base==0x1f||base==0x23||base==0xe){
        const char*nm=(const char*)(lc+off+*(const uint32_t*)(lc+off+8));
        printf("  [%2d] cmd=0x%08x name='%s'\n",n++,cmd,nm);
      }
      off+=cs;
    }
    printf("  (%d dylib-type commands)\n",n);
    return 0;
  }
  printf("not found\n"); return 1;
}
