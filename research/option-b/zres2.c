...
static int is_image(unsigned long p,unsigned long*textvm){
  if(!mapped(p)) return 0;
  if(*(volatile unsigned int*)p!=0xfeedfacf) return 0;
  struct mh64*h=(struct mh64*)p;
  if(h->ncmds==0||h->ncmds>2048||h->sizeofcmds<32||h->sizeofcmds>0x40000) return 0;
  unsigned long q=p+32;
  for(unsigned int i=0;i<h->ncmds && i<8;i++){ struct lc*l=(struct lc*)q;
    if(l->cmd==0x19){ struct seg64*s=(struct seg64*)q;
      if(s->segname[0]=='_'&&s->segname[1]=='_'&&s->segname[2]=='T'&&s->segname[3]=='E'&&s->segname[4]=='X'&&s->segname[5]=='T'){
        if((s->vmaddr&0x3fff)||(s->vmsize==0)||(s->fileoff&0x3fff)) return 0;
        if(s->vmaddr!=p && (s->vmaddr & ~0x3fffUL)!=0x180000000UL){ /* allow cache layout */ }
        if(textvm)*textvm=s->vmaddr; return 1; }
      return 0; }
    if(l->cmdsize<8||l->cmdsize>0x10000) return 0; q+=l->cmdsize; }
  return 0;
}
static unsigned long g_want_hdr; static int g_want_found;
static unsigned long id_of(unsigned long a,char*buf,int n){
  ...
  for(unsigned int c=0;c<h->ncmds;c++){ struct lc*l=(struct lc*)p;
    if(l->cmd==LC_ID_DYLIB){ struct dylibc*d=(struct dylibc*)p; unsigned long np=(unsigned long)d+d->nameoff;
      if(mapped(np)){ for(int i=0;i<n-1;i++){ buf[i]=(char)*(volatile unsigned char*)(np+i); if(!buf[i]) break; } buf[n-1]=0; return np; } }
    if(l->cmdsize<8||l->cmdsize>0x20000) break; p+=l->cmdsize; }
  return 0;
}
// One pass: find the image whose LC_ID_DYLIB equals want; stop at first hit.
static unsigned long find_image(const char*want){
  char buf[128];
  for(unsigned long a=0x180000000UL; a<0x260000000UL; a+=0x4000){
    unsigned long tv;
    if(!is_image(a,&tv)) continue;
    buf[0]=0;
    if(id_of(a,buf,sizeof buf) && seq(buf,want)) return a;
  }
  return 0;
}
...
int main(int argc,char**argv,char**envp){
  ...
  for(unsigned i=0;i<sizeof(T)/sizeof(T[0]);i++){
    ...
    o_lbl(T[i].img); o_lbl(" hdr="); puth(h); o_lbl(" "); o_lbl(T[i].sym); o_lbl("="); puth(r); o_lbl("\n");
  }
  return 0;
}
